...
def plot_rqa_results(
    ...
    N = len(dataX)
    fig = plt.figure(figsize=(8, 9), facecolor="white")  # whole figure white
    gs = gridspec.GridSpec(3, 2, width_ratios=[1, 12], height_ratios=[1, 12, 2],
                           hspace=0.4, wspace=0.2)

    # === Recurrence Plot ===
    ax_rp = fig.add_subplot(gs[1, 1], facecolor="white")  # RP panel white

    recur_y, recur_x = np.where(td == 1)
    ax_rp.scatter(
        recur_x, recur_y,
        c='darkred', s=point_size,
        edgecolors='none',
        rasterized=True  # rasterize the points inside SVG
    )
    ax_rp.set_xlim([0, N])
    ax_rp.set_ylim([0, N])
    ...
    if 'timeseries' in plot_mode:
        ...
        ax_ts_x.plot(np.arange(N), dataX[:N])
    ...
    if dataY is not None and 'timeseries' in plot_mode:
        ...
        ax_ts_y.plot(dataY[:N], np.arange(N))
    ...
    if save_path:
        ...
        # only mkdir if a directory is present
